package com.liansen.common.aop;


import com.liansen.common.constant.CoreConstant;
import com.liansen.common.exception.ExceptionFactory;
...
public class AuthCheckAspect {
    ...
    @Pointcut("execution(public * com.liansen..*.resource.*.*(..))")
    public void webRequestAuth() {}

    @Pointcut("!@within(com.liansen.common.annotation.NotAuth) && !@annotation(com.liansen.common.annotation.NotAuth)")
...
//    @Around("webRequestAuth()&& webRequestNotAuth()")
    public Object doAuth(ProceedingJoinPoint pjp) throws Throwable {
        ...
        String acess_token = request.getHeader("acess_token");
        if(ObjectUtils.isEmpty(token)) {
            token = request.getParameter("token");
        }
...
//            Authentication.setToken(token);
            ...
            Authentication.setAccess_token(acess_token);
